...
if ($count>=2){
   echo "<script>alert('Peminjaman Melebihi Batas Maksimal 2...');</script>";
      echo "<meta http-equiv='refresh' content='0; url=../user.php?page=data_pinjamanggota&id_anggota=$id_anggota'>";
}
$querymax = mysqli_query($connect,"SELECT * FROM tb_reqpinjam WHERE id_anggota='$id_anggota'");
$count =mysqli_num_rows($querymax);
if ($count>=2){
   echo "<script>alert('Request Peminjaman Melebihi Batas yang ditentukan...');</script>";
      echo "<meta http-equiv='refresh' content='0; url=../user.php?page=data_reqpinjam&id_anggota=$id_anggota'>";
}else{
...
$alur=mysqli_query($connect,"SELECT * FROM tb_buku WHERE kd_buku = '$kd_buku'");
  ...
  if ($sisa == 0) {
    ...
    $sql  = 'INSERT INTO tb_reqpinjam(id_anggota,kd_buku,kd_pinjam,tgl_req,status) values ("'.$id_anggota.'","'.$kd_buku.'","'.$kd_pinjam.'","'.$skrg.'","'.$status.'")';
	$query  = mysqli_query($connect,$sql);

  //   $sql  = 'INSERT INTO tb_peminjaman(kd_pinjam,id_anggota,kd_buku,tgl_pinjam) values ("'.$kd_pinjam.'","'.$id_anggota.'","'.$kd_buku.'","'.$skrg.'")';
  // $query  = mysqli_query($connect,$sql);
